...
model_name_or_path = './Training_results_demo'
whisper_checkpoint_path = './whisper-small-checkpoint.pt'
convert_hf_whisper(model_name_or_path, whisper_checkpoint_path)

# now transcribe
import whisper
model = whisper.load_model(whisper_checkpoint_path)
result = model.transcribe('12.wav') # probably longer than 10 min? hour?
print(result['text'])
